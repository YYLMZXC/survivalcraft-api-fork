...
namespace Engine.Content
{
	...
	public class SoundBufferContentWriter : IContentWriter
	{
		...
		[Optional]
		public bool KeepSourceSoundDataInTag;
		...
		public void Write(string projectDirectory, Stream stream)
		{
			using (Stream stream2 = Storage.OpenFile(Storage.CombinePaths(projectDirectory, Sound), OpenFileMode.Read))
			{
				if (Ogg.IsOggStream(stream2))
				{
					if (OggCompressed)
					{
						WriteOgg(stream, Ogg.Stream(stream2), stream2);
					}
					else
					{
						WritePcm(stream, Ogg.Stream(stream2));
					}
					return;
				}
		...
		private void WritePcm(Stream stream, StreamingSource streamingSource)
		{
			BinaryWriter binaryWriter = new BinaryWriter(stream);
			binaryWriter.Write(KeepSourceSoundDataInTag);
		...
		private void WriteOgg(Stream stream, StreamingSource streamingSource, Stream soundStream)
		{
			if (soundStream.Length > int.MaxValue)
			{
				throw new InvalidOperationException("Audio data too long.");
			}
			BinaryWriter binaryWriter = new BinaryWriter(stream);
			...
			binaryWriter.Write(streamingSource.ChannelsCount);
			binaryWriter.Write(streamingSource.SamplingFrequency);
			binaryWriter.Write((int)soundStream.Length);
