...
public abstract class DeciduousLeavesBlock : LeavesBlock
{
	...
	public override BlockPlacementData GetDigValue(SubsystemTerrain subsystemTerrain, ComponentMiner componentMiner, int value, int toolValue, TerrainRaycastResult raycastResult)
	{
		...
		{
			subsystemTerrain.Project.FindSubsystem<SubsystemParticles>(throwOnError: true).AddParticleSystem(new LeavesParticleSystem(subsystemTerrain, raycastResult.CellFace.Point, 8, fadeIn: false, createFallenLeaves: true, value));
			BlockPlacementData result = default(BlockPlacementData);
			result.Value = Terrain.ReplaceData(value, SetSeason(SetIsShaken(data, isManuallyCleared: true), Season.Winter));
	...
	public virtual int SetTimeOfYear(int data, float timeOfYear)
	{
		...
		if (GetIsShaken(data))
		{
			...
			{
				num2 = SetIsShaken(num2, isManuallyCleared: false);
			}
		}
		return num2;
	}
