...
namespace Game
{
	public class ComponentCetaceanBreatheBehavior : ComponentBehavior, IUpdateable
	{
		...
		private Vector3? FindSurfaceDestination()
		{
			...
			Vector3 forward = m_componentCreature.ComponentBody.Matrix.Forward;
			float num = 2f * m_componentCreature.ComponentBody.ImmersionDepth;
			for (int i = 0; i < 16; i++)
			{
				Vector2 vector2 = ((i < 4) ? (new Vector2(forward.X, forward.Z) + m_random.Vector2(0f, 0.25f)) : m_random.Vector2(0.5f, 1f));
				Vector3 vector3 = Vector3.Normalize(new Vector3(vector2.X, 1f, vector2.Y));
				Vector3 end = vector + num * vector3;
