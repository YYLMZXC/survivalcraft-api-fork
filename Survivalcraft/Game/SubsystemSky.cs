...
namespace Game
{
	public class SubsystemSky : Subsystem, IDrawable, IUpdateable
	{
		...
		private int[] m_drawOrders = new int[3] { -100, 5, 105 };

		private float[] m_cloudsLayerRadii = new float[4] { 0f, 0.8f, 0.95f, 1f };

		private Color[] m_cloudsLayerColors = new Color[5];

		private static int[] m_lightValuesMoonless = new int[6] { 0, 3, 6, 9, 12, 15 };

		private static int[] m_lightValuesNormal = new int[6] { 3, 5, 8, 10, 13, 15 };

		public float SkyLightIntensity { get; private set; }

		public int MoonPhase { get; private set; }

		public int SkyLightValue { get; private set; }

		public float VisibilityRange { get; private set; }

		public float VisibilityRangeYMultiplier { get; private set; }

		public float ViewUnderWaterDepth { get; private set; }

		public float ViewUnderMagmaDepth { get; private set; }
		...
		{
			if (!m_skyDomes.TryGetValue(camera.GameWidget, out var value))
			{
				value = new SkyDome();
		...
		private void DrawLightning(Camera camera)
		{
			...
			for (int i = 0; i < (int)(m_lightningStrikeBrightness * 30f); i++)
			{
				...
				while (num4 > value.Y)
				{
					uint num5 = MathUtils.Hash((uint)(m_lightningStrikePosition.Value.X + 100f * m_lightningStrikePosition.Value.Z + 200f * num4));
					float num6 = MathUtils.Lerp(4f, 10f, (float)(num5 & 0xFFu) / 255f);
					float num7 = (((num5 & 1) == 0) ? 1 : (-1));
					float num8 = MathUtils.Lerp(0.05f, 0.2f, (float)((num5 >> 8) & 0xFFu) / 255f);
					float num9 = num4;
					float num10 = num9 - num6 * MathUtils.Lerp(0.45f, 0.55f, (float)((num5 >> 16) & 0xFFu) / 255f);
					float num11 = num9 - num6 * MathUtils.Lerp(0.45f, 0.55f, (float)((num5 >> 24) & 0xFFu) / 255f);
					float num12 = num9 - num6;
					Vector3 p = new Vector3(value.X, num9, value.Z) + vector2;
					Vector3 vector3 = new Vector3(value.X, num10, value.Z) + vector2 - num6 * vector * num7 * num8;
					Vector3 vector4 = new Vector3(value.X, num11, value.Z) + vector2 + num6 * vector * num7 * num8;
					Vector3 p2 = new Vector3(value.X, num12, value.Z) + vector2;
					Color color = Color.White * 0.2f * MathUtils.Saturate((260f - num9) * 0.2f);
					Color color2 = Color.White * 0.2f * MathUtils.Saturate((260f - num10) * 0.2f);
					Color color3 = Color.White * 0.2f * MathUtils.Saturate((260f - num11) * 0.2f);
					Color color4 = Color.White * 0.2f * MathUtils.Saturate((260f - num12) * 0.2f);
					flatBatch3D.QueueLine(p, vector3, color, color2);
					flatBatch3D.QueueLine(vector3, vector4, color2, color3);
					flatBatch3D.QueueLine(vector4, p2, color3, color4);
					num4 -= num6;
				}
			}
			float num13 = MathUtils.Lerp(0.3f, 0.75f, 0.5f * (float)MathUtils.Sin(MathUtils.Remainder(1.0 * m_subsystemTime.GameTime, 6.2831854820251465)) + 0.5f);
			m_lightningStrikeBrightness -= m_subsystemTime.GameTimeDelta / num13;
		...
		private void DrawClouds(Camera camera)
		{
			...
			Vector3 viewPosition = camera.ViewPosition;
			Vector2 vector = new Vector2((float)MathUtils.Remainder(0.0020000000949949026 * gameTime - (double)(viewPosition.X / 1900f * 1.75f), 1.0) + viewPosition.X / 1900f * 1.75f, (float)MathUtils.Remainder(0.0020000000949949026 * gameTime - (double)(viewPosition.Z / 1900f * 1.75f), 1.0) + viewPosition.Z / 1900f * 1.75f);
			...
			for (int i = 0; i < 7; i++)
			{
				for (int j = 0; j < 7; j++)
				{
					...
					Vector2 texCoord = new Vector2(position.X, position.Z) / 1900f * 1.75f - vector;
		...
		private void QueueCelestialBody(TexturedBatch3D batch, Vector3 viewPosition, Color color, float distance, float radius, float angle)
		{
			if (color.A > 0)
			{
				...
				Vector3 vector3 = Vector3.Cross(unitZ, vector2);
				Vector3 p = viewPosition + vector2 * distance - radius * unitZ - radius * vector3;
				Vector3 p2 = viewPosition + vector2 * distance + radius * unitZ - radius * vector3;
				Vector3 p3 = viewPosition + vector2 * distance + radius * unitZ + radius * vector3;
				Vector3 p4 = viewPosition + vector2 * distance - radius * unitZ + radius * vector3;
		...
		private Color CalculateSkyColor(Vector3 direction, float timeOfDay, float precipitationIntensity, int temperature)
		{
			...
			Vector3 v = new Vector3(0.65f, 0.68f, 0.7f) * num;
			...
			Vector3 v4 = Vector3.Lerp(v2, v, precipitationIntensity) * num;
			Vector3 vector2 = Vector3.Lerp(v3, v, precipitationIntensity) * num;
			...
			float num2 = MathUtils.Lerp(CalculateDawnGlowIntensity(timeOfDay), 0f, precipitationIntensity);
			float num3 = MathUtils.Lerp(CalculateDuskGlowIntensity(timeOfDay), 0f, precipitationIntensity);
			...
			return new Color(Vector3.Lerp(vector2 + vector3 * num4 + vector4 * num5, v4, f2));
		}
		...
		private void FillStarsBuffers()
		{
			...
			for (int i = 0; i < 150; i++)
			{
				...
				float num = 9f * random.NormalFloat(1f, 0.1f);
				...
				Vector3 vector = 900f * v;
				Vector3 vector2 = Vector3.Normalize(Vector3.Cross((v.X > v.Y) ? Vector3.UnitY : Vector3.UnitX, v));
				Vector3 vector3 = Vector3.Normalize(Vector3.Cross(vector2, v));
				Vector3 position = vector + num * (-vector2 - vector3);
				Vector3 position2 = vector + num * (vector2 - vector3);
				Vector3 position3 = vector + num * (vector2 + vector3);
				Vector3 position4 = vector + num * (-vector2 + vector3);
