...
namespace Game
{
	public class FurnitureBlock : Block, IPaintableBlock, IElectricElementBlock
	{
		...
		public override void DrawBlock(PrimitivesRenderer3D primitivesRenderer, int value, Color color, float size, ref Matrix matrix, DrawBlockEnvironmentData environmentData)
		{
			...
			vector.X = -0.5f * (float)(design.Box.Left + design.Box.Right) / (float)design.Resolution;
			vector.Y = -0.5f * (float)(design.Box.Top + design.Box.Bottom) / (float)design.Resolution;
			vector.Z = -0.5f * (float)(design.Box.Near + design.Box.Far) / (float)design.Resolution;
			Matrix matrix2 = Matrix.CreateTranslation(vector * size) * matrix;
		...
		public override CraftingRecipe GetAdHocCraftingRecipe(SubsystemTerrain terrain, string[] ingredients, float heatLevel, float playerLevel)
		{
			...
			for (int i = 0; i < ingredients.Length; i++)
			{
				...
				CraftingRecipesManager.DecodeIngredient(ingredients[i], out var craftingId, out var data);
				if (craftingId == BlocksManager.Blocks[227].CraftingId)
				{
					FurnitureDesign design = terrain.SubsystemFurnitureBlockBehavior.GetDesign(GetDesignIndex(data ?? 0));
					if (design == null)
					{
						return null;
					}
