...
namespace Game
{
	public class PlayerData : IDisposable
	{
		...
		public SubsystemPlayers SubsystemPlayers { get; private set; }
		...
		public double FirstSpawnTime { get; private set; }

		public double LastSpawnTime { get; private set; }
		...
		public bool IsDefaultName { get; private set; }
		...
		public PlayerData(Project project)
		{
			...
			ResetName();
			List<WidgetInputDevice> list = new List<WidgetInputDevice> { WidgetInputDevice.Keyboard | WidgetInputDevice.Mouse };
			for (int i = 0; i < 4; i++)
			{
				if (GamePad.IsConnected(i))
				{
					list.Add((WidgetInputDevice)(2048 << i));
				}
			}
			InputDevice = list.Where((WidgetInputDevice d) => SubsystemPlayers.PlayersData.All((PlayerData pd) => (pd.InputDevice & d) == 0)).FirstOrDefault();
			m_stateMachine.AddState("FirstUpdate", null, delegate
			{
				if (ComponentPlayer != null)
				{
					UpdateSpawnDialog(string.Format("Spawning {0} (Level {1})", new object[2]
					{
						Name,
						MathUtils.Floor(Level)
					}), null, 0f, resetProgress: true);
			...
			m_stateMachine.AddState("PrepareSpawn", delegate
			{
				...
				{
					UpdateSpawnDialog(string.Format("Respawning {0} (Level {1})", new object[2]
					{
						Name,
						MathUtils.Floor(Level)
					}), "Your items will remain for a short time at the place of death.", 0f, resetProgress: true);
				}
				else
				{
					UpdateSpawnDialog(string.Format("Spawning {0} (Level {1})", new object[2]
			...
			m_stateMachine.AddState("PlayerDead", delegate
			{
				...
				if (ComponentPlayer != null)
				{
					...
					string text2 = $"Cause of death: {text}";
					if (m_subsystemGameInfo.WorldSettings.GameMode == GameMode.Cruel)
					{
						ComponentPlayer.ComponentGui.DisplayLargeMessage("YOU HAVE DIED", string.Format("{0}\n\nCannot respawn in \"{1}\" game mode", new object[2]
						{
							text2,
							m_subsystemGameInfo.WorldSettings.GameMode.ToString()
						}), 30f, 1.5f);
					}
					...
					{
						ComponentPlayer.ComponentGui.DisplayLargeMessage("YOU HAVE DIED", $"{text2}\n\nTap to restart adventure", 30f, 1.5f);
					}
					else
					{
