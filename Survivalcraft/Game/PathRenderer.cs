...
internal static class PathRenderer
{
	public struct Point : IEquatable<Point>
	{
		...
		{
			if (Position == other.Position && InnerRadiusL == other.InnerRadiusL && InnerRadiusR == other.InnerRadiusR && OuterRadiusL == other.OuterRadiusL && OuterRadiusR == other.OuterRadiusR && InnerColorL == other.InnerColorL && InnerColorR == other.InnerColorR && OuterColorL == other.OuterColorL && OuterColorR == other.OuterColorR && LengthScale == other.LengthScale)
			{
	...
	public static void GeneratePathNormals(DynamicArray<Vector2> positions, DynamicArray<bool> visibility, bool loop, DynamicArray<Vector2> normals)
	{
		...
		if (array2 != null)
		{
			if (loop)
			{
				for (int i = 0; i < positions.Count; i++)
				{
					...
					normals.Add((array2[num] || array2[num2]) ? Normal2(array[num2] - array[num], array[num3] - array[num2]) : default(Vector2));
				}
				return;
			}
			normals.Add(array2[0] ? Normal1(array[1] - array[0]) : default(Vector2));
			for (int j = 1; j < positions.Count - 1; j++)
			{
				normals.Add((array2[j - 1] || array2[j]) ? Normal2(array[j] - array[j - 1], array[j + 1] - array[j]) : default(Vector2));
			}
			if (positions.Count >= 2)
			{
				normals.Add(array2[positions.Count - 2] ? Normal1(array[positions.Count - 1] - array[positions.Count - 2]) : default(Vector2));
			}
		}
		else if (loop)
		{
	...
	public static void QueuePath(FlatBatch2D batch, DynamicArray<Point> points, DynamicArray<bool> visibility, bool loop, bool flatShading, float depth = 0f)
	{
		...
		QueuePathInternal(batch, points, TmpNormals, visibility, loop, flatShading, depth);
	}

	public static void QueuePath(FlatBatch2D batch, DynamicArray<Point> points, DynamicArray<Vector2> normals, DynamicArray<bool> visibility, bool loop, bool flatShading, float depth = 0f)
	{
		QueuePathInternal(batch, points, normals, visibility, loop, flatShading, depth);
	}

	public static void QueuePath(FlatBatch2D batch, DynamicArray<Vector2> positions, DynamicArray<bool> visibility, bool loop, bool flatShading, float innerRadius, float outerRadius, Color innerColor, Color outerColor, float miterLimit, float depth = 0f)
	{
		QueuePath(batch, positions, visibility, loop, flatShading, innerRadius, innerRadius, outerRadius, outerRadius, innerColor, innerColor, outerColor, outerColor, miterLimit, depth);
	}

	public static void QueuePath(FlatBatch2D batch, DynamicArray<Vector2> positions, DynamicArray<bool> visibility, bool loop, bool flatShading, float innerRadiusL, float innerRadiusR, float outerRadiusL, float outerRadiusR, Color innerColorL, Color innerColorR, Color outerColorL, Color outerColorR, float miterLimit, float depth = 0f)
	{
		...
		QueuePathInternal(batch, TmpPoints, TmpNormals, visibility, loop, flatShading, depth);
	}

	public static void QueuePath(TexturedBatch2D batch, DynamicArray<Point> points, DynamicArray<bool> visibility, bool loop, bool flatShading, float lengthOffset, float depth = 0f)
	{
		TmpPositions.Count = 0;
		foreach (Point point in points)
		{
		...
		TmpNormals.Count = 0;
		GeneratePathNormals(TmpPositions, visibility, loop, TmpNormals);
		QueuePathInternal(batch, points, TmpNormals, visibility, loop, flatShading, lengthOffset, depth);
	}

	public static void QueuePath(TexturedBatch2D batch, DynamicArray<Point> points, DynamicArray<Vector2> normals, DynamicArray<bool> visibility, bool loop, bool flatShading, float lengthOffset, float depth = 0f)
	{
		QueuePathInternal(batch, points, normals, visibility, loop, flatShading, lengthOffset, depth);
	}

	public static void QueuePath(TexturedBatch2D batch, DynamicArray<Vector2> positions, DynamicArray<bool> visibility, bool loop, bool flatShading, float innerRadius, float outerRadius, Color innerColor, Color outerColor, float lengthScale, float lengthOffset, float miterLimit, float depth = 0f)
	{
		QueuePath(batch, positions, visibility, loop, flatShading, innerRadius, innerRadius, outerRadius, outerRadius, innerColor, innerColor, outerColor, outerColor, lengthScale, lengthOffset, miterLimit, depth);
	}

	public static void QueuePath(TexturedBatch2D batch, DynamicArray<Vector2> positions, DynamicArray<bool> visibility, bool loop, bool flatShading, float innerRadiusL, float innerRadiusR, float outerRadiusL, float outerRadiusR, Color innerColorL, Color innerColorR, Color outerColorL, Color outerColorR, float lengthScale, float lengthOffset, float miterLimit, float depth = 0f)
	{
		Point point = default(Point);
		point.InnerRadiusL = innerRadiusL;
		point.InnerRadiusR = innerRadiusR;
	...
	private static void QueuePathInternal(FlatBatch2D batch, DynamicArray<Point> points, DynamicArray<Vector2> normals, DynamicArray<bool> visibility, bool loop, bool flatShading, float depth)
	{
		...
		for (int i = 0; i < num; i++)
		{
			if (visibility != null && !visibility[i])
			{
				continue;
			}
			int index = i;
			...
			if (point.OuterRadiusL != 0f || point2.OuterRadiusL != 0f)
			{
				...
				if (flatShading)
				{
					batch.QueueQuad(position + vector4, position + vector3, position2 + vector5, position2 + vector6, 0f, point.OuterColorR, point.InnerColorR, point.InnerColorR, point.OuterColorR);
				}
				else
				{
					batch.QueueQuad(position + vector4, position + vector3, position2 + vector5, position2 + vector6, 0f, point.OuterColorR, point.InnerColorR, point2.InnerColorR, point2.OuterColorR);
				}
			}
			if (point.OuterRadiusR != 0f || point2.OuterRadiusR != 0f)
			{
				...
				if (flatShading)
				{
					batch.QueueQuad(position + vector8, position + vector7, position2 + vector9, position2 + vector10, 0f, point.OuterColorR, point.InnerColorR, point.InnerColorR, point.OuterColorR);
				}
				else
				{
					batch.QueueQuad(position + vector8, position + vector7, position2 + vector9, position2 + vector10, 0f, point.OuterColorR, point.InnerColorR, point2.InnerColorR, point2.OuterColorR);
				}
			}
		}
	}
	...
	private static void QueuePathInternal(TexturedBatch2D batch, DynamicArray<Point> points, DynamicArray<Vector2> normals, DynamicArray<bool> visibility, bool loop, bool flatShading, float lengthOffset, float depth)
	{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			if (point.OuterRadiusL > 0f || point2.OuterRadiusL > 0f)
			{
				...
				{
					batch.QueueQuad(position + vector7, position + vector6, position2 + vector8, position2 + vector9, 0f, vector4 + vector11, vector4 + vector10, vector5 + vector12, vector5 + vector13, point.OuterColorR, point.InnerColorR, point2.InnerColorR, point2.OuterColorR);
				}
			}
			if (point.OuterRadiusR != 0f || point2.OuterRadiusR != 0f)
			{
				...
				{
					batch.QueueQuad(position + vector15, position + vector14, position2 + vector16, position2 + vector17, 0f, vector4 + vector19, vector4 + vector18, vector5 + vector20, vector5 + vector21, point.OuterColorR, point.InnerColorR, point2.InnerColorR, point2.OuterColorR);
				}
			}
		}
	}
