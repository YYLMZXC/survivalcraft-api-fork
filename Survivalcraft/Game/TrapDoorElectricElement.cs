			...
			if (!m_needsReset)
			{
				if (num >= 10)
				{
					if (IsSignalHigh(m_voltage))
					{
						...
						SubsystemElectricity.Project.FindSubsystem<SubsystemTrapdoorBlockBehavior>(throwOnError: true).OpenCloseTrapdoor(cellFace.X, cellFace.Y, cellFace.Z, !TrapdoorBlock.GetOpen(data));
					}
				}
				else
				{
