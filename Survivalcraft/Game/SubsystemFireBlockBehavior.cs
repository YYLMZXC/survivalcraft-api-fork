...
namespace Game
{
	public class SubsystemFireBlockBehavior : SubsystemBlockBehavior, IUpdateable
	{
		...
		public override int[] HandledBlocks => new int[1] { 104 };
		...
		public void Update(float dt)
		{
			...
			if (m_firePointsCopy.Count > 0)
			{
				...
				{
					if (m_fireData.TryGetValue(m_firePointsCopy.Array[m_copyIndex], out var value))
					{
						int x = value.Point.X;
						int y = value.Point.Y;
						int z = value.Point.Z;
		...
		public override void OnNeighborBlockChanged(int x, int y, int z, int neighborX, int neighborY, int neighborZ)
		{
			...
			if (((uint)num & (true ? 1u : 0u)) != 0 && BlocksManager.Blocks[base.SubsystemTerrain.Terrain.GetCellContents(x, y, z + 1)].FireDuration == 0f)
			{
				num &= -2;
			}
			if (((uint)num & 2u) != 0 && BlocksManager.Blocks[base.SubsystemTerrain.Terrain.GetCellContents(x + 1, y, z)].FireDuration == 0f)
			{
				num &= -3;
			}
			if (((uint)num & 4u) != 0 && BlocksManager.Blocks[base.SubsystemTerrain.Terrain.GetCellContents(x, y, z - 1)].FireDuration == 0f)
			{
				num &= -5;
			}
			if (((uint)num & 8u) != 0 && BlocksManager.Blocks[base.SubsystemTerrain.Terrain.GetCellContents(x - 1, y, z)].FireDuration == 0f)
			{
				num &= -9;
			}
			if (m_fireData.TryGetValue(new Point3(x, y, z), out var value))
			{
				if (((uint)num & (true ? 1u : 0u)) != 0 && neighborX == x && neighborY == y && neighborZ == z + 1)
				{
					InitializeFireDataTime(value, 0);
				}
				if (((uint)num & 2u) != 0 && neighborX == x + 1 && neighborY == y && neighborZ == z)
				{
					InitializeFireDataTime(value, 1);
				}
				if (((uint)num & 4u) != 0 && neighborX == x && neighborY == y && neighborZ == z - 1)
				{
					InitializeFireDataTime(value, 2);
				}
				if (((uint)num & 8u) != 0 && neighborX == x - 1 && neighborY == y && neighborZ == z)
				{
