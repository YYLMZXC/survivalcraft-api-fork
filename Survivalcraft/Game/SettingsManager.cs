			...
			try
			{
				...
				using (Stream stream = Storage.OpenFile("data:/Settings.xml", OpenFileMode.Read))
				{
					foreach (XElement item in XmlUtils.LoadXmlFromStream(stream, null, throwOnError: true).Elements())
					{
						...
						{
							Log.Warning(string.Format("Setting \"{0}\" could not be loaded. Reason: {1}", new object[2] { name, ex.Message }));
						}
					}
				}
		...
		public static void SaveSettings()
		{
			try
			{
					...
					select pi)
				{
					...
					{
						Log.Warning(string.Format("Setting \"{0}\" could not be saved. Reason: {1}", new object[2] { item.Name, ex.Message }));
					}
				}
