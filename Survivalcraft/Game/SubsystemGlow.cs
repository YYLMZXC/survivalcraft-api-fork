		...
		public void Draw(Camera camera, int drawOrder)
		{
			foreach (GlowPoint key in m_glowPoints.Keys)
			{
				...
				if (num2 < m_subsystemSky.ViewFogRange.Y)
				{
					...
					Vector3 p = key.Position + num3 * (-key.Right - key.Up) + vector2;
					Vector3 p2 = key.Position + num3 * (key.Right - key.Up) + vector2;
					Vector3 p3 = key.Position + num3 * (key.Right + key.Up) + vector2;
					Vector3 p4 = key.Position + num3 * (-key.Right + key.Up) + vector2;
