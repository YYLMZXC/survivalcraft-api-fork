...
namespace Game
{
	public class PrecipitationShaftParticleSystem : ParticleSystemBase
	{
		...
		private double m_lastUpdateTime = double.MinValue;

		private float m_lastSkylightIntensity = float.MinValue;
		...
		public override void Draw(Camera camera)
		{
			...
			Vector3 obj = ((m_precipitationType == PrecipitationType.Rain) ? Vector3.UnitY : Vector3.Normalize(Vector3.Cross(viewDirection, vector)));
			Vector3 vector2 = vector * m_size.X;
			Vector3 vector3 = obj * m_size.Y;
			if (m_precipitationType == PrecipitationType.Rain)
			{
				...
				for (int i = 0; i < m_particles.Length; i++)
				{
					...
					if (particle.IsActive)
					{
						Vector3 p = particle.Position + vector4;
						Vector3 p2 = particle.Position + vector5;
						Vector3 p3 = particle.Position + vector6;
			...
			Vector3 vector8 = vector2 - vector3;
			Vector3 vector9 = vector2 + vector3;
			Vector3 vector10 = -vector2 + vector3;
			for (int j = 0; j < m_particles.Length; j++)
			{
				Particle particle2 = m_particles[j];
				if (particle2.IsActive)
				{
					Vector3 p4 = particle2.Position + vector7;
					Vector3 p5 = particle2.Position + vector8;
					Vector3 p6 = particle2.Position + vector9;
					Vector3 p7 = particle2.Position + vector10;
		...
		private void Initialize()
		{
			...
			case PrecipitationType.Snow:
			{
				...
				for (int i = 0; i < m_particles.Length; i++)
				{
					...
					Vector2 vector = new Vector2((int)particle.TextureSlot % 4, (int)particle.TextureSlot / 4) * num;
					particle.TexCoord1 = vector + new Vector2(0f, 0f);
					particle.TexCoord2 = vector + new Vector2(num, 0f);
					particle.TexCoord3 = vector + new Vector2(num, num);
					particle.TexCoord4 = vector + new Vector2(0f, num);
