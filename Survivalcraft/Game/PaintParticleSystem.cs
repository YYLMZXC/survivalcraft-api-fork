	...
	public class PaintParticleSystem : ParticleSystem<PaintParticleSystem.Particle>
	{
			...
			: base(20)
		{
			...
			for (int i = 0; i < base.Particles.Length; i++)
			{
				...
				obj.IsActive = true;
				Vector2 vector3 = new Vector2(m_random.Float(-1f, 1f), m_random.Float(-1f, 1f));
				obj.Position = position + 0.4f * (vector3.X * vector + vector3.Y * vector2) + 0.03f * normal;
				obj.Color = m_color;
				obj.Size = new Vector2(m_random.Float(0.025f, 0.035f));
				obj.TimeToLive = m_random.Float(0.5f, 1.5f);
				obj.Velocity = 1f * (vector3.X * vector + vector3.Y * vector2) + m_random.Float(-3f, 0.5f) * normal;
				obj.TextureSlot = 15;
