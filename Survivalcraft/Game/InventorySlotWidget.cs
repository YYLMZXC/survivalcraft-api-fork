...
namespace Game
{
	public class InventorySlotWidget : CanvasWidget, IDragTargetWidget
	{
		...
		public InventorySlotWidget()
		{
			...
			{
				Text = "Split",
				Font = ContentManager.Get<BitmapFont>("Fonts/Pericles12"),
		...
		private bool HandleDragDrop(IInventory sourceInventory, int sourceSlotIndex, DragMode dragMode, IInventory targetInventory, int targetSlotIndex)
		{
			...
			if (slotProcessCapacity > 0)
			{
				...
				targetInventory.ProcessSlotItems(targetSlotIndex, slotValue, num, processCount, out var processedValue, out var processedCount);
