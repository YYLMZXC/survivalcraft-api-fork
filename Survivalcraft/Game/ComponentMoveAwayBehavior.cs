		...
		public override void Load(ValuesDictionary valuesDictionary, IdToEntityMap idToEntityMap)
		{
			...
			m_stateMachine.AddState("Move", delegate
			{
				...
				if (m_target != null)
				{
					...
					float num4 = m_random.Float(1.5f, 2f);
					float speed = (m_isFast ? 0.7f : 0.35f);
					m_componentPathfinding.SetDestination(m_componentCreature.ComponentBody.Position + num4 * new Vector3(vector2.X, 0f, vector2.Y), speed, 1f, 0, useRandomMovements: false, ignoreHeightDifference: true, raycastDestination: false, null);
				}
			}, delegate
			{
				if (!IsActive)
