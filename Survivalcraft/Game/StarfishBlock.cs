		...
		public override void Initialize()
		{
			...
			for (int i = 0; i < 6; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					...
					m_blockMeshes[num2].AppendModelMeshPart(model.FindMesh("Starfish").MeshParts[0], boneAbsoluteTransform * matrix, makeEmissive: false, flipWindingOrder: false, doubleSided: false, flipNormals: false, Color.White);
					m_blockMeshes[num2].TransformTextureCoordinates(Matrix.CreateTranslation((float)(num % 16) / 16f, (float)(num / 16) / 16f, 0f));
					m_collisionBoxes[num2] = new BoundingBox[1] { m_blockMeshes[num2].CalculateBoundingBox() };
				}
			}
