...
namespace Game
{
	public class ComponentInput : Component, IUpdateable
	{
		...
		public bool IsControlledByTouch { get; private set; } = true;
		...
		public IInventory SplitSourceInventory { get; private set; }

		public int SplitSourceSlotIndex { get; private set; }
		...
		public Ray3? CalculateVrHandRay()
		{
			if (VrManager.IsControllerPresent(VrController.Right))
			{
				...
				Matrix matrix = VrManager.HmdMatrixInverted * Matrix.CreateWorld(activeCamera.ViewPosition, activeCamera.ViewDirection, activeCamera.ViewUp);
				Matrix matrix2 = VrManager.GetControllerMatrix(VrController.Right) * matrix;
				return new Ray3(matrix2.Translation + matrix2.Forward * (5f / 64f), matrix2.Forward);
			}
			return null;
		}
		...
		private void UpdateInputFromMouseAndKeyboard(WidgetInput input)
		{
			...
			else
			{
				...
				vector += Vector3.UnitX * (input.IsKeyDown(Key.D) ? 1 : 0);
		...
		private void UpdateInputFromWidgets(WidgetInput input)
		{
			...
			Vector2 vector = new Vector2(SettingsManager.LeftHandedLayout ? 96 : (-96), -96f);
			...
			if (m_componentGui.ViewWidget != null && m_componentGui.ViewWidget.TouchInput.HasValue)
			{
				...
				else if (value.InputType == TouchInputType.Move)
				{
					...
					{
						Vector2 vector2 = Vector2.TransformNormal(value.Move, m_componentGui.ViewWidget.InvertedGlobalTransform);
						Vector2 vector3 = num2 / num3 * new Vector2(0.0006f, -0.0006f) * vector2 * MathUtils.Pow(vector2.LengthSquared(), 0.125f);
						m_playerInput.Look += vector3;
					}
			...
			if (m_componentGui.MoveWidget != null && m_componentGui.MoveWidget.TouchInput.HasValue)
			{
				...
				{
					Vector2 vector4 = Vector2.TransformNormal(value2.Move, m_componentGui.ViewWidget.InvertedGlobalTransform);
					Vector2 vector5 = num / num3 * new Vector2(0.003f, -0.003f) * vector4 * MathUtils.Pow(vector4.LengthSquared(), 0.175f);
					m_playerInput.CrouchMove.X += vector5.X;
					m_playerInput.CrouchMove.Z += vector5.Y;
					Vector2 vector6 = Vector2.TransformNormal(value2.TotalMoveLimited, m_componentGui.ViewWidget.InvertedGlobalTransform);
					m_playerInput.Move.X += ProcessInputValue(vector6.X * viewWidget.GlobalScale, 0.2f * radius, radius);
					m_playerInput.Move.Z += ProcessInputValue((0f - vector6.Y) * viewWidget.GlobalScale, 0.2f * radius, radius);
				}
			}
			...
			if (m_componentGui.LookWidget != null && m_componentGui.LookWidget.TouchInput.HasValue)
			{
				...
				{
					Vector2 vector7 = Vector2.TransformNormal(value3.Move, m_componentGui.ViewWidget.InvertedGlobalTransform);
					Vector2 vector8 = num2 / num3 * new Vector2(0.0006f, -0.0006f) * vector7 * MathUtils.Pow(vector7.LengthSquared(), 0.125f);
					m_playerInput.Look += vector8;
				}
			}
		}
