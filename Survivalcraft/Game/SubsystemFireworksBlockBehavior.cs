...
namespace Game
{
	public class SubsystemFireworksBlockBehavior : SubsystemBlockBehavior, IUpdateable
	{
		...
		public void ExplodeFireworks(Vector3 position, int data)
		{
			for (int i = 0; i < 3; i++)
			{
				Vector3 vector = new Vector3(m_random.Float(-3f, 3f), -15f, m_random.Float(-3f, 3f));
				if (m_subsystemTerrain.Raycast(position, position + vector, useInteractionBoxes: false, skipAirBlocks: true, null).HasValue)
				{
					return;
				}
			}
