	...
	public class ComponentHumanModel : ComponentCreatureModel
	{
		...
		public override void Animate()
		{
			...
			if (m_lieDownFactorModel == 0f)
			{
				...
				float num13 = ((((uint)m_punchCounter & (true ? 1u : 0u)) != 0) ? num11 : 0f);
				float num14 = 0f;
				float num15 = 0f;
				float num16 = 0f;
				float num17 = 0f;
				...
				float num25 = MathUtils.Min(12f * m_subsystemTime.GameTimeDelta, 1f);
				m_headAngles += num25 * (new Vector2(x, y) - m_headAngles);
				m_handAngles1 += num25 * (new Vector2(num4, num5) - m_handAngles1);
				m_handAngles2 += num25 * (new Vector2(num6, num7) - m_handAngles2);
				m_legAngles1 += num25 * (new Vector2(num3, y2) - m_legAngles1);
				m_legAngles2 += num25 * (new Vector2(x2, y3) - m_legAngles2);
			...
			{
				float num26 = MathUtils.Max(base.DeathPhase, m_lieDownFactorModel);
				float num27 = 1f - num26;
				Vector3 position2 = position + num26 * 0.5f * m_componentCreature.ComponentBody.BoxSize.Y * Vector3.Normalize(m_componentCreature.ComponentBody.Matrix.Forward * new Vector3(1f, 0f, 1f)) + num26 * Vector3.UnitY * m_componentCreature.ComponentBody.BoxSize.Z * 0.1f;
				SetBoneTransform(m_bodyBone.Index, Matrix.CreateFromYawPitchRoll(vector.X, (float)Math.PI / 2f * num26, 0f) * Matrix.CreateTranslation(position2));
				SetBoneTransform(m_headBone.Index, Matrix.Identity);
				SetBoneTransform(m_hand1Bone.Index, Matrix.CreateRotationY(m_handAngles1.Y * num27) * Matrix.CreateRotationX(m_handAngles1.X * num27));
				SetBoneTransform(m_hand2Bone.Index, Matrix.CreateRotationY(m_handAngles2.Y * num27) * Matrix.CreateRotationX(m_handAngles2.X * num27));
				SetBoneTransform(m_leg1Bone.Index, Matrix.CreateRotationY(m_legAngles1.Y * num27) * Matrix.CreateRotationX(m_legAngles1.X * num27));
				SetBoneTransform(m_leg2Bone.Index, Matrix.CreateRotationY(m_legAngles2.Y * num27) * Matrix.CreateRotationX(m_legAngles2.X * num27));
			}
			base.Animate();
		}
		...
		public override void DrawExtras(Camera camera)
		{
			if (m_componentCreature.ComponentHealth.Health > 0f && m_componentMiner != null && m_componentMiner.ActiveBlockValue != 0)
			{
				...
				Matrix matrix = base.AbsoluteBoneTransformsForCamera[m_hand2Bone.Index];
				...
				Matrix matrix2 = Matrix.CreateRotationY(MathUtils.DegToRad(block.InHandRotation.Y) + m_inHandItemRotation.Y) * Matrix.CreateRotationZ(MathUtils.DegToRad(block.InHandRotation.Z) + m_inHandItemRotation.Z) * Matrix.CreateRotationX(MathUtils.DegToRad(block.InHandRotation.X) + m_inHandItemRotation.X) * Matrix.CreateTranslation(block.InHandOffset + m_inHandItemOffset) * Matrix.CreateTranslation(new Vector3(0.05f, 0.05f, -0.56f) * (m_componentCreature.ComponentBody.BoxSize.Y / 1.77f)) * matrix;
				...
				block.DrawBlock(m_subsystemModelsRenderer.PrimitivesRenderer, m_componentMiner.ActiveBlockValue, Color.White, block.InHandScale, ref matrix3, m_drawBlockEnvironmentData);
			}
