	...
	public abstract class StairsBlock : Block, IPaintableBlock
	{
		...
		public override void Initialize()
		{
			...
			for (int i = 0; i < 24; i++)
			{
				...
				Matrix matrix = ((!isUpsideDown) ? (Matrix.CreateRotationY((float)rotation * (float)Math.PI / 2f) * Matrix.CreateTranslation(0.5f, 0f, 0.5f)) : (Matrix.CreateRotationY((float)rotation * (float)Math.PI / 2f) * Matrix.CreateTranslation(0.5f, -0.5f, 0.5f) * Matrix.CreateScale(1f, -1f, 1f) * Matrix.CreateTranslation(0f, 0.5f, 0f)));
				BlockMesh blockMesh = new BlockMesh();
				switch (cornerType)
				{
				case CornerType.None:
					blockMesh.AppendModelMeshPart(model.FindMesh("Stairs").MeshParts[0], boneAbsoluteTransform * matrix, makeEmissive: false, isUpsideDown, doubleSided: false, flipNormals: false, Color.White);
					break;
				case CornerType.OneQuarter:
					blockMesh.AppendModelMeshPart(model.FindMesh("StairsOuterCorner").MeshParts[0], boneAbsoluteTransform2 * matrix, makeEmissive: false, isUpsideDown, doubleSided: false, flipNormals: false, Color.White);
					break;
				case CornerType.ThreeQuarters:
					blockMesh.AppendModelMeshPart(model.FindMesh("StairsInnerCorner").MeshParts[0], boneAbsoluteTransform3 * matrix, makeEmissive: false, isUpsideDown, doubleSided: false, flipNormals: false, Color.White);
					break;
				}
