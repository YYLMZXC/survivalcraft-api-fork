	...
	public class VersionConverter22To23 : VersionConverter
	{
		...
		public override void ConvertWorld(string directoryName)
		{
			try
			{
				...
				foreach (string item2 in from f in Storage.ListDirectoryNames(directoryName)
					where Storage.GetExtension(f) == ".new"
					select f)
				{
				...
				foreach (string item4 in from f in Storage.ListDirectoryNames(directoryName)
					where Storage.GetExtension(f) == ".old"
					select f)
				{
			...
			catch (Exception ex)
			{
				...
				foreach (string item6 in from f in Storage.ListDirectoryNames(directoryName)
					where Storage.GetExtension(f) == ".old"
					select f)
				{
				...
				foreach (string item7 in from f in Storage.ListFileNames(directoryName)
					where Storage.GetExtension(f) == ".new"
					select f)
				{
					Storage.DeleteFile(Storage.CombinePaths(directoryName, item7));
				}
				foreach (string item8 in from f in Storage.ListDirectoryNames(directoryName)
					where Storage.GetExtension(f) == ".new"
					select f)
				{
		...
		private void ConvertChunks(string directoryName)
		{
			...
			using (TerrainSerializer22 terrainSerializer = new TerrainSerializer22(directoryName))
			{
