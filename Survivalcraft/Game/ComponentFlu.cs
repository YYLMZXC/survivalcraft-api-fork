	...
	public class ComponentFlu : Component, IUpdateable
	{
			...
			else if (m_componentPlayer.ComponentVitalStats.Temperature < 6f)
			{
				...
				if (m_fluOnset > 120f)
				{
					...
					if (m_subsystemTime.GameTime - m_lastMessageTime > 60.0)
					{
						...
						m_componentPlayer.ComponentGui.DisplaySmallMessage("Get warm, you will get flu!", Color.White, blinking: true, playNotificationSound: true);
					}
				}
		...
		private void FluEffect()
		{
			...
			if (injury > 0f)
			{
				...
				{
					m_componentPlayer.ComponentHealth.Injure(injury, null, ignoreInvulnerability: false, "Flu");
				});
			}
			if (Time.FrameStartTime - m_lastMessageTime > 60.0)
			{
				...
				m_subsystemTime.QueueGameTimeDelayedExecution(m_subsystemTime.GameTime + 1.5, delegate
				{
					...
					{
						m_componentPlayer.ComponentGui.DisplaySmallMessage("Get to warmth, you have flu!", Color.White, blinking: true, playNotificationSound: true);
					}
					else
					{
						m_componentPlayer.ComponentGui.DisplaySmallMessage("You have flu, warmth is healing", Color.White, blinking: true, playNotificationSound: true);
					}
				});
			}
