...
namespace Game
{
	public class GunSmokeParticleSystem : ParticleSystem<GunSmokeParticleSystem.Particle>
	{
		...
		public override bool Simulate(float dt)
		{
			...
			float num3 = MathUtils.Lerp(20f, 0f, MathUtils.Saturate(2f * m_time / 0.5f));
			Vector3 vector = new Vector3(2f, 2f, 1f);
			if (m_time < 0.5f)
			{
				m_toGenerate += num * dt;
			}
			...
			for (int i = 0; i < base.Particles.Length; i++)
			{
				...
				else if (m_toGenerate >= 1f)
				{
					...
					Vector3 vector2 = m_random.Vector3(0f, 1f);
					particle.Position = m_position + 0.3f * vector2;
					particle.Color = m_color;
					particle.Velocity = num3 * (m_direction + m_random.Vector3(0f, 0.1f)) + 2.5f * vector2;
