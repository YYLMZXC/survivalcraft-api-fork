			...
			if (!m_needsReset)
			{
				if (num >= 10)
				{
					if (IsSignalHigh(m_voltage))
					{
						...
						SubsystemElectricity.Project.FindSubsystem<SubsystemSpikesBlockBehavior>(throwOnError: true).RetractExtendSpikes(cellFace.X, cellFace.Y, cellFace.Z, !SpikedPlankBlock.GetSpikesState(data));
					}
				}
				else
				{
