...
namespace Game
{
	public class ClothingBlock : Block
	{
		...
		private static Matrix[] m_slotTransforms = new Matrix[4]
		{
		...
		public override void Initialize()
		{
			...
			foreach (XElement item in ContentManager.Get<XElement>("Clothes").Elements())
			{
		...
		public override string GetDisplayName(SubsystemTerrain subsystemTerrain, int value)
		{
			...
			if (clothingColor != 0)
			{
				return SubsystemPalette.GetName(subsystemTerrain, clothingColor, "dyed " + clothingData.DisplayName);
			}
			...
			if (list.Count == 2)
			{
				...
				if (num != 0 && num3 != 0)
				{
					...
					if (num4 != clothingColor)
					{
						...
						craftingRecipe.ResultValue = block2.SetDamage(Terrain.MakeBlockValue(203, 0, SetClothingIndex(SetClothingColor(0, num4), clothingIndex)), damage);
						craftingRecipe.RemainsCount = 1;
						craftingRecipe.RemainsValue = BlocksManager.DamageItem(Terrain.MakeBlockValue(129, 0, color), damage2 + MathUtils.Max(block.Durability / 4, 1));
						craftingRecipe.RequiredHeatLevel = 1f;
						craftingRecipe.Description = $"Dye clothing {SubsystemPalette.GetName(terrain, color, null)}";
						craftingRecipe.Ingredients = (string[])ingredients.Clone();
		...
		{
			return m_clothingData[GetClothingIndex(data)];
		}
