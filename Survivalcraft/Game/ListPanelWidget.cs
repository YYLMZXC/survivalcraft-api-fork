...
namespace Game
{
	public class ListPanelWidget : ScrollPanelWidget
	{
		...
		public override void Draw(DrawContext dc)
		{
			...
			{
				Vector2 vector = ((Direction != 0) ? new Vector2(0f, (float)SelectedIndex.Value * ItemSize - ScrollPosition) : new Vector2((float)SelectedIndex.Value * ItemSize - ScrollPosition, 0f));
				FlatBatch2D flatBatch2D = dc.PrimitivesRenderer2D.FlatBatch(0, DepthStencilState.None);
				int count = flatBatch2D.TriangleVertices.Count;
				Vector2 vector2 = ((Direction == LayoutDirection.Horizontal) ? new Vector2(ItemSize, base.ActualSize.Y) : new Vector2(base.ActualSize.X, ItemSize));
				flatBatch2D.QueueQuad(vector, vector + vector2, 0f, SelectionColor * base.GlobalColorTransform);
		...
		private void CreateListWidgets(float size)
		{
			...
			for (int i = m_firstVisibleIndex; i <= m_lastVisibleIndex; i++)
			{
				...
				if (!m_widgetsByIndex.TryGetValue(i, out var value))
				{
