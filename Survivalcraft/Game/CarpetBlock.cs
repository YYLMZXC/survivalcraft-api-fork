			...
			if (list.Count == 2)
			{
				...
				if (num != 0 && num3 != 0)
				{
					...
					if (num5 != num4)
					{
						...
						craftingRecipe.ResultValue = Terrain.MakeBlockValue(208, 0, num5);
						craftingRecipe.RemainsCount = 1;
						craftingRecipe.RemainsValue = BlocksManager.DamageItem(Terrain.MakeBlockValue(129, 0, color), damage + MathUtils.Max(block.Durability / 4, 1));
						craftingRecipe.RequiredHeatLevel = 1f;
						craftingRecipe.Description = $"Dye carpet {SubsystemPalette.GetName(subsystemTerrain, color, null)}";
