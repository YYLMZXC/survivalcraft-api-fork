	...
	public class SubsystemPathfinding : Subsystem
	{
		...
		private class World : IAStarWorld<Vector3>
		{
			...
			private void AddNeighbor(DynamicArray<Vector3> neighbors, Vector3 p, int dx, int dz)
			{
				...
				Vector3 vector2 = new Vector3((float)num2 + 0.5f, num + 0.01f, (float)num4 + 0.5f);
				Vector3 vector3 = 1f / (float)num8 * (vector2 - vector);
				for (int i = 1; i <= num8; i++)
				{
					Vector3 vector4 = vector + i * vector3;
					BoundingBox box = new BoundingBox(vector4 - new Vector3(Request.BoxSize.X / 2f + 0.01f, 0f, Request.BoxSize.Z / 2f + 0.01f), vector4 + new Vector3(Request.BoxSize.X / 2f - 0.01f, Request.BoxSize.Y, Request.BoxSize.Z / 2f - 0.01f));
					if (IsBlocked(box))
					{
						return;
					}
					...
					for (int j = num3; j <= num6; j++)
					{
						...
						while (num8 <= num5)
						{
							...
							if (num9 != 0)
							{
								...
								if (block.IsCollidable)
								{
									Vector3 vector = new Vector3(i, num8, j);
									...
									for (int k = 0; k < customCollisionBoxes.Length; k++)
									{
										...
										if (box.Intersection(new BoundingBox(vector + boundingBox.Min, vector + boundingBox.Max)))
										{
											return true;
										}
									}
		...
		private bool IsPassable(Vector3 p1, Vector3 p2, Vector3 boxSize)
		{
			...
			Vector3 vector3 = (0.5f * boxSize.X + 0.1f) * Vector3.Normalize(Vector3.Cross(Vector3.UnitY, vector2 - vector));
			...
			if (m_subsystemTerrain.Raycast(vector - vector3, vector2 - vector3, useInteractionBoxes: false, skipAirBlocks: true, SmoothingRaycastFunction_Obstacle).HasValue)
			{
				return false;
			}
			if (m_subsystemTerrain.Raycast(vector + vector3, vector2 + vector3, useInteractionBoxes: false, skipAirBlocks: true, SmoothingRaycastFunction_Obstacle).HasValue)
			...
			if (m_subsystemTerrain.Raycast(vector + new Vector3(0f, -1f, 0f) - vector3, vector2 + new Vector3(0f, -1f, 0f) - vector3, useInteractionBoxes: false, skipAirBlocks: false, SmoothingRaycastFunction_Support).HasValue)
			{
				return false;
			}
			if (m_subsystemTerrain.Raycast(vector + new Vector3(0f, -1f, 0f) + vector3, vector2 + new Vector3(0f, -1f, 0f) + vector3, useInteractionBoxes: false, skipAirBlocks: false, SmoothingRaycastFunction_Support).HasValue)
