...
namespace Game
{
	public class ComponentLocomotion : Component, IUpdateable
	{
		...
		public float LadderSpeed { get; private set; }

		public float JumpSpeed { get; private set; }

		public float FlySpeed { get; private set; }

		public float CreativeFlySpeed { get; private set; }
		...
		public float InAirWalkFactor { get; private set; }
		...
		public int? LadderValue { get; private set; }
		...
		public Vector2? LastWalkOrder { get; private set; }

		public float LastJumpOrder { get; private set; }

		public Vector3? LastFlyOrder { get; private set; }

		public Vector3? LastSwimOrder { get; private set; }

		public Vector2 LastTurnOrder { get; private set; }
		...
		private void NormalMovement(float dt)
		{
			...
			if (WalkSpeed > 0f && WalkOrder.HasValue)
			{
				...
				{
					Vector3 vector2 = new Vector3(WalkOrder.Value.X, 0f, WalkOrder.Value.Y);
					if (FlyOrder.HasValue)
					{
						vector2 += FlyOrder.Value;
					}
					Vector3 vector3 = ((!SettingsManager.HorizontalCreativeFlight || m_componentPlayer == null || m_componentPlayer.ComponentInput.IsControlledByTouch) ? Vector3.Normalize(vector + 0.1f * Vector3.UnitY) : Vector3.Normalize(vector * new Vector3(1f, 0f, 1f)));
					Vector3 vector4 = CreativeFlySpeed * (right * vector2.X + Vector3.UnitY * vector2.Y + vector3 * vector2.Z);
					float num = ((vector2 == Vector3.Zero) ? 5f : 3f);
					velocity += MathUtils.Saturate(num * dt) * (vector4 - velocity);
				...
				else
				{
					...
					Vector3 vector5 = value.X * Vector3.Normalize(new Vector3(right.X, 0f, right.Z)) + value.Y * Vector3.Normalize(new Vector3(vector.X, 0f, vector.Z));
					Vector3 vector6 = num2 * vector5 + m_componentCreature.ComponentBody.StandingOnVelocity;
					float num4;
					...
					velocity.X += num4 * (vector6.X - velocity.X);
					velocity.Z += num4 * (vector6.Z - velocity.Z);
					Vector3 vector7 = value.X * right + value.Y * vector;
					if (m_componentLevel != null)
					{
						vector7 *= m_componentLevel.SpeedFactor;
					}
					velocity.Y += 10f * AccelerationFactor * vector7.Y * m_componentCreature.ComponentBody.ImmersionFactor * dt;
					m_componentCreature.ComponentBody.IsGroundDragEnabled = false;
					if (m_componentPlayer != null && Time.PeriodicEvent(10.0, 0.0) && (m_shoesWarningTime == 0.0 || Time.FrameStartTime - m_shoesWarningTime > 300.0) && m_componentCreature.ComponentBody.StandingOnValue.HasValue && m_componentCreature.ComponentBody.ImmersionFactor < 0.1f)
					{
						...
						if (!flag && vector6.LengthSquared() / velocity.LengthSquared() > 0.99f && WalkOrder.Value.LengthSquared() > 0.99f)
						{
			...
			if (FlySpeed > 0f && FlyOrder.HasValue)
			{
				...
				Vector3 vector8 = FlySpeed * value3;
				velocity += MathUtils.Saturate(2f * AccelerationFactor * dt) * (vector8 - velocity);
				m_componentCreature.ComponentBody.IsGravityEnabled = false;
				m_flying = true;
			}
			if (SwimSpeed > 0f && SwimOrder.HasValue && m_componentCreature.ComponentBody.ImmersionFactor > 0.5f)
			{
				...
				Vector3 vector9 = SwimSpeed * value4;
				float num5 = 2f;
				if (value4.LengthSquared() >= 0.99f)
				{
					vector9 *= MathUtils.Lerp(1f, 2f, m_swimBurstRemaining);
					num5 *= MathUtils.Lerp(1f, 4f, m_swimBurstRemaining);
					m_swimBurstRemaining -= dt;
				}
				velocity += MathUtils.Saturate(num5 * AccelerationFactor * dt) * (vector9 - velocity);
			...
			if (JumpOrder > 0f && (m_componentCreature.ComponentBody.StandingOnValue.HasValue || m_componentCreature.ComponentBody.ImmersionFactor > 0.5f) && m_componentCreature.ComponentBody.CrouchFactor == 0f)
			{
