...
namespace Game
{
	public class ComponentClothing : Component, IUpdateable, IInventory
	{
		...
		public float Insulation { get; private set; }

		public ClothingSlot LeastInsulatedSlot { get; private set; }
			...
			if (m_subsystemGameInfo.WorldSettings.GameMode != 0 && m_subsystemGameInfo.WorldSettings.AreAdventureSurvivalMechanicsEnabled && m_subsystemTime.PeriodicGameTimeEvent(0.5, 0.0))
			{
				foreach (int enumValue in EnumUtils.GetEnumValues(typeof(ClothingSlot)))
				{
					...
					while (num < m_clothesList.Count)
					{
						...
						{
							m_componentGui.DisplaySmallMessage(string.Format("Must be level {0} to wear {1}", new object[2] { clothingData.PlayerLevelRequired, clothingData.DisplayName }), Color.White, blinking: true, playNotificationSound: true);
		...
		{
			ClothingData clothingData = ClothingBlock.GetClothingData(Terrain.ExtractData(value));
			ReadOnlyList<int> clothes = GetClothes(clothingData.Slot);
			if (clothes.Count == 0)
			{
				return true;
			}
			ClothingData clothingData2 = ClothingBlock.GetClothingData(Terrain.ExtractData(clothes[clothes.Count - 1]));
