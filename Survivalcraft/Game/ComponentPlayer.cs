...
namespace Game
{
	public class ComponentPlayer : ComponentCreature, IUpdateable
	{
		...
		public ComponentBlockHighlight ComponentBlockHighlight { get; private set; }

		public ComponentScreenOverlays ComponentScreenOverlays { get; private set; }

		public ComponentAimingSights ComponentAimingSights { get; private set; }
		...
		public void Update(float dt)
		{
			...
			{
				base.ComponentLocomotion.WalkOrder = ((base.ComponentBody.CrouchFactor > 0f) ? (0.66f * new Vector2(playerInput.CrouchMove.X, playerInput.CrouchMove.Z)) : new Vector2(playerInput.Move.X, playerInput.Move.Z));
				...
				if (!m_isAimBlocked)
				{
					...
					if (ComponentInput.IsControlledByVr || (vector.X >= (float)size.X * 0.02f && vector.X < (float)size.X * 0.98f && vector.Y >= (float)size.Y * 0.02f && vector.Y < (float)size.Y * 0.98f))
					{
						...
						else if (!m_aimHintIssued && Time.PeriodicEvent(1.0, 0.0))
						{
							...
							{
								if (!m_aimHintIssued && m_aim.HasValue && base.ComponentBody.CrouchFactor == 0f)
								{
									m_aimHintIssued = true;
									ComponentGui.DisplaySmallMessage("Crouch to improve your aim", Color.White, blinking: true, playNotificationSound: true);
								}
							});
						}
					}
			...
			if (playerInput.Drop && ComponentMiner.Inventory != null)
			{
				...
				{
					Vector3 position = base.ComponentBody.Position + new Vector3(0f, base.ComponentBody.StanceBoxSize.Y * 0.66f, 0f) + 0.25f * base.ComponentBody.Matrix.Forward;
			...
			if (num5 == 0 && !block2.IsNonDuplicable)
			{
				...
				block2.GetDropValues(m_subsystemTerrain, value3, 0, int.MaxValue, list, out var _);
