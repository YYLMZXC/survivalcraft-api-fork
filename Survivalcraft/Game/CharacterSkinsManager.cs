...
namespace Game
{
	public static class CharacterSkinsManager
	{
		...
		private static string CharacterSkinsDirectoryName => "data:/CharacterSkins";
		...
		public static Texture2D LoadTexture(string name)
		{
			...
			{
				Log.Warning(string.Format("Could not load character skin \"{0}\". Reason: {1}.", new object[2] { name, ex.Message }));
			}
			if (texture2D == null)
			{
		...
		{
			if (!m_playerModels.TryGetValue(playerClass, out var value))
			{
		...
		{
			if (!m_outerClothingModels.TryGetValue(playerClass, out var value))
			{
				ValuesDictionary valuesDictionary;
				switch (playerClass)
				{
		...
		private static void ValidateCharacterSkin(Stream stream)
		{
			...
			{
				throw new InvalidOperationException(string.Format("Character skin is larger than 256x256 pixels (size={0}x{1})", new object[2] { image.Width, image.Height }));
			}
			if (!MathUtils.IsPowerOf2(image.Width) || !MathUtils.IsPowerOf2(image.Height))
			{
				throw new InvalidOperationException(string.Format("Character skin does not have power-of-two size (size={0}x{1})", new object[2] { image.Width, image.Height }));
			}
		}
	}
}
