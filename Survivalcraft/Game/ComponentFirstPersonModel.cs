		...
		public void Draw(Camera camera, int drawOrder)
		{
			...
			try
			{
				...
				if (m_value != 0)
				{
					...
					Matrix matrix2 = Matrix.CreateFromYawPitchRoll(vector.Y, vector.X, vector.Z) * identity * Matrix.CreateTranslation(position3) * Matrix.CreateFromYawPitchRoll(m_lagAngles.X, m_lagAngles.Y, 0f) * matrix * camera.ViewMatrix;
					...
					block.DrawBlock(m_primitivesRenderer, m_value, Color.White, block.FirstPersonScale, ref matrix2, m_drawBlockEnvironmentData);
				...
				Matrix matrix3 = Matrix.CreateScale(0.01f) * Matrix.CreateRotationX(0.8f) * Matrix.CreateRotationY(0.4f) * identity * Matrix.CreateTranslation(position4) * Matrix.CreateFromYawPitchRoll(m_lagAngles.X, m_lagAngles.Y, 0f) * matrix * camera.ViewMatrix;
				...
				m_shader.Transforms.World[0] = matrix3;
