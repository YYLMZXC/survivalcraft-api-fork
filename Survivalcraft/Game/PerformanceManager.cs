	...
	public static class PerformanceManager
	{
		...
		static PerformanceManager()
		{
			...
			}, delegate
			{
				if (GameManager.Project != null)
				{
					if (ScreensManager.CurrentScreen != null && (object)ScreensManager.CurrentScreen.GetType() == typeof(GameScreen))
					{
			...
			m_stateMachine.AddState("PostMeasure", delegate
			{
				if (m_frameCount > 0)
				{
					...
					AnalyticsManager.LogEvent("[PerformanceManager] Measurement", new AnalyticsParameter("FrameCount", m_frameCount.ToString()), new AnalyticsParameter("AverageFrameTime", num + "ms"), new AnalyticsParameter("AverageFrameCpuTime", num2 + "ms"));
					Log.Information(string.Format("PerformanceManager Measurement: frames={0}, avgFrameTime={1}ms, avgFrameCpuTime={2}ms", new object[3]
					{
						m_frameCount.ToString(),
						num.ToString(),
						num2.ToString()
					}));
				}
		...
		public static void Draw()
		{
			...
			if (SettingsManager.DisplayFpsRibbon)
			{
				...
				float num4 = 0.5f;
				int num5 = MathUtils.Max((int)((float)viewport.Width / num), 1);
				if (m_frameData == null || m_frameData.Length != num5)
				{
					m_frameData = new FrameData[num5];
					m_frameDataIndex = 0;
				}
				...
				Color color = Color.Orange * num4;
				Color color2 = Color.Red * num4;
				for (int num6 = m_frameData.Length - 1; num6 >= 0; num6--)
				{
					int num7 = (num6 - m_frameData.Length + 1 + m_frameDataIndex + m_frameData.Length) % m_frameData.Length;
					FrameData frameData = m_frameData[num7];
					float x = (float)num6 * num;
					float x2 = (float)(num6 + 1) * num;
