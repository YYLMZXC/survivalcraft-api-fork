...
namespace Game
{
	public class SubsystemSpawn : Subsystem, IUpdateable
	{
		...
		{
			m_chunks.TryGetValue(point, out var value);
			return value;
		}
		...
		private void UpdateLastVisitedTime()
		{
			...
			{
				Vector2 vector = new Vector2(componentPlayer.ComponentBody.Position.X, componentPlayer.ComponentBody.Position.Z);
				Vector2 p = vector - new Vector2(8f);
				Vector2 p2 = vector + new Vector2(8f);
		...
		private void SpawnChunks()
		{
			...
			foreach (GameWidget gameWidget in m_subsystemViews.GameWidgets)
			{
				...
				Vector2 p = vector - new Vector2(40f);
				Vector2 p2 = vector + new Vector2(40f);
				Point2 point = Terrain.ToChunk(p);
				Point2 point2 = Terrain.ToChunk(p2);
				for (int i = point.X; i <= point2.X; i++)
				{
					for (int j = point.Y; j <= point2.Y; j++)
					{
						Vector2 v = new Vector2(((float)i + 0.5f) * 16f, ((float)j + 0.5f) * 16f);
						if (!(Vector2.DistanceSquared(vector, v) < 1600f))
						{
							continue;
						}
						TerrainChunk chunkAtCell = m_subsystemTerrain.Terrain.GetChunkAtCell(Terrain.ToCell(v.X), Terrain.ToCell(v.Y));
		...
		private Entity SpawnEntity(SpawnEntityData data)
		{
			...
			{
				Log.Error(string.Format("Unable to spawn entity with template \"{0}\". Reason: {1}", new object[2] { data.TemplateName, ex.Message }));
				return null;
			}
		}
