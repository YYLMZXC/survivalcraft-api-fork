...
namespace Game
{
	public class SubsystemCreatureSpawn : Subsystem, IUpdateable
	{
		...
		private const int m_totalLimitConstant = 6;

		private const int m_totalLimitConstantChallenging = 16;
		...
		private void InitializeCreatureTypes()
		{
			m_creatureTypes.Add(new CreatureType("Duck", SpawnLocationType.Surface, randomSpawn: true, constantSpawn: false)
			{
				...
				{
					float num101 = m_subsystemTerrain.TerrainContentsGenerator.CalculateOceanShoreDistance(point.X, point.Z);
					...
					int num102 = Terrain.ExtractContents(m_subsystemTerrain.Terrain.GetCellValueFast(point.X, point.Y - 1, point.Z));
					int topHeight5 = m_subsystemTerrain.Terrain.GetTopHeight(point.X, point.Z);
					return (humidity26 > 8 && temperature40 > 4 && num101 > 30f && point.Y >= topHeight5 && (BlocksManager.Blocks[num102] is LeavesBlock || num102 == 18 || num102 == 8 || num102 == 2)) ? 2f : 0f;
				},
			...
			m_creatureTypes.Add(new CreatureType("Raven", SpawnLocationType.Surface, randomSpawn: true, constantSpawn: false)
			{
				SpawnSuitabilityFunction = delegate(CreatureType creatureType, Point3 point)
				{
					float num99 = m_subsystemTerrain.TerrainContentsGenerator.CalculateOceanShoreDistance(point.X, point.Z);
					...
					int num100 = Terrain.ExtractContents(m_subsystemTerrain.Terrain.GetCellValueFast(point.X, point.Y - 1, point.Z));
					int topHeight4 = m_subsystemTerrain.Terrain.GetTopHeight(point.X, point.Z);
					return ((humidity25 <= 8 || temperature39 <= 4) && num99 > 30f && point.Y >= topHeight4 && (BlocksManager.Blocks[num100] is LeavesBlock || num100 == 62 || num100 == 8 || num100 == 2 || num100 == 7)) ? 2f : 0f;
				},
			...
			m_creatureTypes.Add(new CreatureType("Seagull", SpawnLocationType.Surface, randomSpawn: true, constantSpawn: false)
			{
				SpawnSuitabilityFunction = delegate(CreatureType creatureType, Point3 point)
				{
					...
					return (num97 > -100f && num97 < 40f && point.Y >= topHeight3 && (num98 == 18 || num98 == 7 || num98 == 6 || num98 == 62)) ? 2f : 0f;
				},
			...
			m_creatureTypes.Add(new CreatureType("Pigeon", SpawnLocationType.Surface, randomSpawn: true, constantSpawn: false)
			{
				SpawnSuitabilityFunction = delegate(CreatureType creatureType, Point3 point)
				{
					...
					return (temperature38 > 3 && num95 > 30f && point.Y >= topHeight2 && (BlocksManager.Blocks[num96] is LeavesBlock || num96 == 8 || num96 == 2 || num96 == 7)) ? 2f : 0f;
				},
			...
			m_creatureTypes.Add(new CreatureType("Sparrow", SpawnLocationType.Surface, randomSpawn: true, constantSpawn: false)
			{
				SpawnSuitabilityFunction = delegate(CreatureType creatureType, Point3 point)
				{
					...
					return (temperature37 > 3 && num93 > 20f && point.Y >= topHeight && (BlocksManager.Blocks[num94] is LeavesBlock || num94 == 8 || num94 == 2 || num94 == 7)) ? 3f : 0f;
				},
		...
		private void SpawnRandomCreature()
		{
			...
			foreach (GameWidget gameWidget in m_subsystemViews.GameWidgets)
			{
				...
				if (CountCreaturesInArea(vector - new Vector2(60f), vector + new Vector2(60f), constantSpawn: false) >= num)
				{
					break;
				}
		...
		{
			int num = ((m_subsystemGameInfo.WorldSettings.GameMode >= GameMode.Challenging) ? 16 : 6);
			int num2 = (constantSpawn ? num : 24);
			int num3 = (constantSpawn ? 4 : 3);
			float v = (constantSpawn ? 42 : 16);
			int num4 = CountCreatures(constantSpawn);
			...
			int num5 = CountCreaturesInArea(c2, c3, constantSpawn);
			for (int i = 0; i < maxAttempts; i++)
			{
				...
				if (spawnPoint.HasValue)
				{
					...
					{
						CreatureType creatureType = source.ElementAt(randomWeightedItem);
						int num6 = creatureType.SpawnFunction(creatureType, spawnPoint.Value);
						num4 += num6;
						num5 += num6;
					}
				}
		...
		private Entity SpawnCreature(string templateName, Vector3 position, bool constantSpawn)
		{
			...
			{
				Log.Error(string.Format("Unable to spawn creature with template \"{0}\". Reason: {1}", new object[2] { templateName, ex.Message }));
				return null;
			}
		}
