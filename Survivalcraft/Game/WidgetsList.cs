...
namespace Game
{
	...
	{
		public struct Enumerator : IEnumerator<Widget>, IEnumerator, IDisposable
		{
		...
		public Widget Find(string name, Type type, bool throwIfNotFound = true)
		{
			...
			{
				if ((name == null || (widget2.Name != null && widget2.Name == name)) && ((object)type == null || (object)type == widget2.GetType() || widget2.GetType().GetTypeInfo().IsSubclassOf(type)))
				{
					return widget2;
				}
			...
			if (throwIfNotFound)
			{
				throw new Exception(string.Format("Required widget \"{0}\" of type \"{1}\" not found.", new object[2] { name, type }));
			}
			return null;
		}
