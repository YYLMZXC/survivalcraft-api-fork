...
namespace Game
{
	public class BusyBarWidget : Widget
	{
		...
		public override void Draw(DrawContext dc)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				Vector2 vector = new Vector2(((float)i + 0.5f) * 24f, 6f);
				Color color = ((i == m_boxIndex % 5) ? LitBarColor : UnlitBarColor);
				float v = ((i == m_boxIndex % 5) ? 12f : 8f);
				flatBatch2D.QueueQuad(vector - new Vector2(v) / 2f, vector + new Vector2(v) / 2f, 0f, color * base.GlobalColorTransform);
			}
