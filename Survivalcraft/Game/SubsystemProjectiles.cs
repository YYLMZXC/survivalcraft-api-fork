...
namespace Game
{
	public class SubsystemProjectiles : Subsystem, IUpdateable, IDrawable
	{
		...
		private static int[] m_drawOrders = new int[1] { 10 };
		...
		public Projectile FireProjectile(int value, Vector3 position, Vector3 velocity, Vector3 angularVelocity, ComponentCreature owner)
		{
			...
			Vector3 vector = Vector3.Normalize(velocity);
			Vector3 vector2 = position;
			if (owner != null)
			{
				Ray3 ray = new Ray3(position + vector * 5f, -vector);
				...
				if (num2.HasValue)
				{
					...
					vector2 = position + vector * (5f - num2.Value + 0.1f);
				}
			}
			Vector3 end = vector2 + vector * block.ProjectileTipOffset;
			...
			{
				Projectile projectile = AddProjectile(value, vector2, velocity, angularVelocity, owner);
		...
		public void Update(float dt)
		{
			...
			foreach (Projectile projectile in m_projectiles)
			{
				...
				Vector3 vector2 = block.ProjectileTipOffset * Vector3.Normalize(projectile.Velocity);
				BodyRaycastResult? bodyRaycastResult = m_subsystemBodies.Raycast(position + vector2, vector + vector2, 0.2f, (ComponentBody body, float distance) => true);
				TerrainRaycastResult? terrainRaycastResult = m_subsystemTerrain.Raycast(position + vector2, vector + vector2, useInteractionBoxes: false, skipAirBlocks: true, (int value, float distance) => BlocksManager.Blocks[Terrain.ExtractContents(value)].IsCollidable);
				...
				else if (terrainRaycastResult.HasValue)
				{
					...
					if (projectile.IsIncendiary)
					{
						...
						Vector3 vector4 = projectile.Position - 0.75f * Vector3.Normalize(projectile.Velocity);
						for (int k = 0; k < 8; k++)
						{
							Vector3 vector5 = ((k == 0) ? Vector3.Normalize(projectile.Velocity) : m_random.Vector3(1.5f));
							TerrainRaycastResult? terrainRaycastResult2 = m_subsystemTerrain.Raycast(vector4, vector4 + vector5, useInteractionBoxes: false, skipAirBlocks: true, (int value, float distance) => true);
					...
					{
						Vector3 vector6 = Vector3.Normalize(projectile.Velocity);
						float num3 = MathUtils.Lerp(0.1f, 0.2f, MathUtils.Saturate((num2 - 15f) / 20f));
						vector3 = position + terrainRaycastResult.Value.Distance * Vector3.Normalize(projectile.Velocity) + vector6 * num3;
					}
					else
					{
						...
						projectile.Velocity = num4 * Vector3.Normalize(projectile.Velocity + m_random.Vector3(num4 / 6f, num4 / 3f));
				...
				if (terrainRaycastResult.HasValue || bodyRaycastResult.HasValue)
				{
					...
					else if (!projectile.ToRemove && (vector3.HasValue || projectile.Velocity.Length() < 1f))
					{
						if (projectile.ProjectileStoppedAction == ProjectileStoppedAction.TurnIntoPickable)
						{
							int num5 = BlocksManager.DamageItem(projectile.Value, 1);
							if (num5 != 0)
							{
								if (vector3.HasValue)
								{
									CalculateVelocityAlignMatrix(block, vector3.Value, projectile.Velocity, out var matrix);
									m_subsystemPickables.AddPickable(num5, 1, projectile.Position, Vector3.Zero, matrix);
								}
								else
								{
									m_subsystemPickables.AddPickable(num5, 1, position, Vector3.Zero, null);
								}
							}
							projectile.ToRemove = true;
						}
				...
				float num6 = (projectile.IsInWater ? MathUtils.Pow(0.001f, dt) : MathUtils.Pow(block.ProjectileDamping, dt));
				projectile.Velocity.Y += -10f * dt;
				projectile.Velocity *= num6;
				projectile.AngularVelocity *= num6;
				...
				if (projectile.TrailParticleSystem != null)
				{
					...
					Vector3 vector7 = ((projectile.TrailOffset != Vector3.Zero) ? Vector3.TransformNormal(projectile.TrailOffset, Matrix.CreateFromAxisAngle(Vector3.Normalize(projectile.Rotation), projectile.Rotation.Length())) : Vector3.Zero);
					projectile.TrailParticleSystem.Position = projectile.Position + vector7;
				...
				if (projectile.IsInWater != flag2)
				{
					if (flag2)
					{
						float num7 = new Vector2(projectile.Velocity.X + projectile.Velocity.Z).Length();
						if (num7 > 6f && num7 > 4f * MathUtils.Abs(projectile.Velocity.Y))
						{
