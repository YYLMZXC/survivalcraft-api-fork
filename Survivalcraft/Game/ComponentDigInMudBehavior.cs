...
namespace Game
{
	public class ComponentDigInMudBehavior : ComponentBehavior, IUpdateable
	{
		...
		private double m_digOutTime = double.NegativeInfinity;
		...
		private Vector3? FindDestination()
		{
			for (int i = 0; i < 8; i++)
			{
				...
				Vector3 vector2 = Vector3.Normalize(new Vector3(vector.X, y, vector.Y));
				Vector3 vector3 = m_componentCreature.ComponentBody.Position + m_random.Float(8f, 16f) * vector2;
				TerrainRaycastResult? terrainRaycastResult = m_subsystemTerrain.Raycast(m_componentCreature.ComponentBody.Position, vector3, useInteractionBoxes: false, skipAirBlocks: false, delegate(int value, float d)
				{
				...
				{
					return m_componentCreature.ComponentBody.Position + vector2 * terrainRaycastResult.Value.Distance;
				}
			}
			return null;
		}
