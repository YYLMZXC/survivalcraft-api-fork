...
namespace Game
{
	public class HitValueParticleSystem : ParticleSystem<HitValueParticleSystem.Particle>
	{
		...
		public override void Draw(Camera camera)
		{
			if (m_batch == null)
			{
				m_batch = SubsystemParticles.PrimitivesRenderer.FontBatch(ContentManager.Get<BitmapFont>("Fonts/Pericles18"), 0, DepthStencilState.None);
			}
			Vector3 viewDirection = camera.ViewDirection;
			Vector3 vector = Vector3.Normalize(Vector3.Cross(viewDirection, Vector3.UnitY));
			Vector3 vector2 = -Vector3.Normalize(Vector3.Cross(vector, viewDirection));
			for (int i = 0; i < base.Particles.Length; i++)
			{
				Particle particle = base.Particles[i];
				if (particle.IsActive)
				{
					...
					if (num4 > 0f)
					{
						float num5 = 0.006f * MathUtils.Sqrt(num);
						Color color = particle.Color * num4;
						m_batch.QueueText(particle.Text, particle.Position, vector * num5, vector2 * num5, color, TextAnchor.Center, Vector2.Zero);
					}
				}
			}
		}
