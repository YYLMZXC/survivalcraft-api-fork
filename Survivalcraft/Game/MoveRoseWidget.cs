...
namespace Game
{
	public class MoveRoseWidget : Widget
	{
		...
		public override void Update()
		{
			...
			m_jump = false;
			Vector2 vector = base.ActualSize / 2f;
			...
			foreach (TouchLocation touchLocation in base.Input.TouchLocations)
			{
				...
				if (touchLocation.State == TouchLocationState.Pressed && Vector2.Distance(ScreenToWidget(touchLocation.Position), vector) <= num2)
				{
					m_jump = true;
				...
				if (touchLocation.State != TouchLocationState.Moved && touchLocation.State != 0)
				{
					continue;
				}
				Vector2 vector2 = ScreenToWidget(touchLocation.Position);
				float num4 = Vector2.Distance(vector2, vector);
				if (num4 > num2 && num4 <= num)
				{
					float num5 = Vector2.Angle(vector2 - vector, -Vector2.UnitY);
