			...
			while (color < 16)
			{
				...
				for (int additive = 0; additive < 4; additive = num2)
				{
					...
					if (num != color)
					{
						...
						craftingRecipe.Description = $"Make {SubsystemPalette.GetName(null, num, null)} Paint";
						craftingRecipe.ResultValue = Terrain.MakeBlockValue(129, 0, num);
						craftingRecipe.ResultCount = 1;
						craftingRecipe.RequiredHeatLevel = 1f;
						CraftingRecipe craftingRecipe2 = craftingRecipe;
						craftingRecipe2.Ingredients[0] = BlocksManager.Blocks[129].CraftingId + ":" + color.ToString(CultureInfo.InvariantCulture);
						craftingRecipe2.Ingredients[1] = additives[additive];
