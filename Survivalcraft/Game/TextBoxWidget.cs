...
namespace Game
{
	internal class TextBoxWidget : Widget
	{
		...
		public override void Draw(DrawContext dc)
		{
			...
			Vector2 vector = new Vector2(0f, base.ActualSize.Y / 2f) + new Vector2(num - m_scroll, 0f);
			if (m_hasFocus)
			{
				if (vector.X < 0f)
				{
					m_scroll = MathUtils.Max(m_scroll + vector.X, 0f);
				}
				if (vector.X > base.ActualSize.X)
				{
					m_scroll += vector.X - base.ActualSize.X + 1f;
				}
			}
			...
			flatBatch2D.QueueQuad(vector - new Vector2(0f, Font.GlyphHeight / 2f * FontScale * Font.Scale), vector + new Vector2(1f, Font.GlyphHeight / 2f * FontScale * Font.Scale), 0f, color);
