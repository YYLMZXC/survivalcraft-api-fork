	...
	public class ComponentBlockHighlight : Component, IDrawable, IUpdateable
	{
		private class Geometry : TerrainGeometry
		{
			public Geometry()
			{
				...
				TerrainGeometrySubset[] array = new TerrainGeometrySubset[6] { terrainGeometrySubset, terrainGeometrySubset, terrainGeometrySubset, terrainGeometrySubset, terrainGeometrySubset, terrainGeometrySubset };
		...
		private static int[] m_drawOrders = new int[2] { 1, 2000 };

		public Point3? NearbyEditableCell { get; private set; }
		...
		public void Update(float dt)
		{
			...
			Ray3? ray = ((!m_componentPlayer.ComponentInput.IsControlledByVr) ? new Ray3?(new Ray3(activeCamera.ViewPosition, activeCamera.ViewDirection)) : m_componentPlayer.ComponentInput.CalculateVrHandRay());
		...
		private void DrawReticleHighlight(Camera camera)
		{
			if (camera.Eye.HasValue && m_highlightRaycastResult is TerrainRaycastResult)
			{
				...
				Vector3 p = vector + num2 * (-vector3 + vector4) + num3 * vector2;
				Vector3 p2 = vector + num2 * (vector3 + vector4) + num3 * vector2;
				Vector3 p3 = vector + num2 * (vector3 - vector4) + num3 * vector2;
				Vector3 p4 = vector + num2 * (-vector3 - vector4) + num3 * vector2;
