	...
	public class LightbulbBlock : MountedElectricElementBlock, IPaintableBlock
	{
		...
		public override void Initialize()
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				m_bulbBlockMeshes[i].AppendModelMeshPart(model.FindMesh("Top").MeshParts[0], boneAbsoluteTransform * matrix, makeEmissive: false, flipWindingOrder: false, doubleSided: false, flipNormals: false, Color.White);
				...
				m_bulbBlockMeshesLit[i].AppendModelMeshPart(model.FindMesh("Top").MeshParts[0], boneAbsoluteTransform * matrix, makeEmissive: true, flipWindingOrder: false, doubleSided: false, flipNormals: false, new Color(255, 255, 230));
				...
				m_sidesBlockMeshes[i].AppendModelMeshPart(model.FindMesh("Sides").MeshParts[0], boneAbsoluteTransform2 * matrix, makeEmissive: false, flipWindingOrder: false, doubleSided: true, flipNormals: false, Color.White);
			...
			m_standaloneBulbBlockMesh.AppendModelMeshPart(model.FindMesh("Top").MeshParts[0], boneAbsoluteTransform * matrix2, makeEmissive: false, flipWindingOrder: false, doubleSided: true, flipNormals: false, Color.White);
			m_standaloneBulbBlockMesh.TransformTextureCoordinates(Matrix.CreateTranslation(0.1875f, 0.25f, 0f));
			m_standaloneSidesBlockMesh.AppendModelMeshPart(model.FindMesh("Sides").MeshParts[0], boneAbsoluteTransform2 * matrix2, makeEmissive: false, flipWindingOrder: false, doubleSided: true, flipNormals: false, Color.White);
		...
		public override void DrawBlock(PrimitivesRenderer3D primitivesRenderer, int value, Color color, float size, ref Matrix matrix, DrawBlockEnvironmentData environmentData)
		{
			...
			Color color3 = (color2.HasValue ? SubsystemPalette.GetColor(environmentData, color2) : m_copperColor);
			BlocksManager.DrawMeshBlock(primitivesRenderer, m_standaloneSidesBlockMesh, color * color3, 2f * size, ref matrix, environmentData);
