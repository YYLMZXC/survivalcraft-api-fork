	...
	public class ComponentSwimAwayBehavior : ComponentBehavior, IUpdateable
	{
		...
		private Vector3 FindSafePlace()
		{
			...
			for (int i = 0; i < 40; i++)
			{
				...
				Vector3 vector4 = vector + m_random.Float(10f, 20f) * vector3;
				...
				Vector3 vector5 = (terrainRaycastResult.HasValue ? (vector + vector3 * terrainRaycastResult.Value.Distance) : vector4);
				float num2 = ScoreSafePlace(vector, vector5, herdPosition);
				if (num2 > num)
				{
					num = num2;
					result = vector5;
