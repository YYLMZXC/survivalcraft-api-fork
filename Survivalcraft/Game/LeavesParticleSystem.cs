...
public class LeavesParticleSystem : ParticleSystem<LeavesParticleSystem.Particle>
{
	...
	private bool m_createFallenLeaves;
				...
				else if (block.IsCollidable && !(block is LeavesBlock))
				{
					...
					if (num8.HasValue && num8 < num7 - 0f)
					{
						...
						if (m_createFallenLeaves)
						{
							...
							m_subsystemTerrain.Project.FindSubsystem<SubsystemDeciduousLeavesBlockBehavior>(throwOnError: true).CreateFallenLeaves(m_point, applyImmediately: true);
						}
					}
				}
			}
