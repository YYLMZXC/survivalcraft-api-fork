...
namespace Game
{
	public class PlayerScreen : Screen
	{
		...
		private class InputDeviceWidget : LabelWidget
		{
			...
			{
				base.Text = GetDeviceDisplayName(Device);
			}
		}
		...
		public static ReadOnlyList<WidgetInputDevice> AllInputDevices => new ReadOnlyList<WidgetInputDevice>(m_allInputDevices);
		...
		public override void Update()
		{
			...
			m_controlsLabel.Text = GetDeviceDisplayName(m_allInputDevices.Where((WidgetInputDevice id) => (id & m_playerData.InputDevice) != 0).FirstOrDefault());
			...
			if (m_characterSkinButton.IsClicked)
			{
				...
				ListSelectionDialog dialog = new ListSelectionDialog("Select Character Skin", items, 64f, delegate(object item)
				{
					...
					containerWidget.Children.Find<LabelWidget>("CharacterSkinItem.Text").Text = CharacterSkinsManager.GetDisplayName((string)item);
					containerWidget.Children.Find<LabelWidget>("CharacterSkinItem.Details").Text = string.Format("{0}x{1}", new object[2] { texture.Width, texture.Height });
			...
			{
				DialogsManager.ShowDialog(null, new ListSelectionDialog("Select Input Device", m_allInputDevices, 56f, (object d) => new InputDeviceWidget
				{
		...
		public static string GetDeviceDisplayName(WidgetInputDevice device)
		{
			...
			case WidgetInputDevice.MultiKeyboard1 | WidgetInputDevice.MultiMouse1:
				return "Mouse & Keyboard 1" + ((MultiMouse.IsConnected(0) && MultiKeyboard.IsConnected(0)) ? "" : " (not found)");
			case WidgetInputDevice.MultiKeyboard2 | WidgetInputDevice.MultiMouse2:
				return "Mouse & Keyboard 2" + ((MultiMouse.IsConnected(1) && MultiKeyboard.IsConnected(1)) ? "" : " (not found)");
			case WidgetInputDevice.MultiKeyboard3 | WidgetInputDevice.MultiMouse3:
				return "Mouse & Keyboard 3" + ((MultiMouse.IsConnected(2) && MultiKeyboard.IsConnected(2)) ? "" : " (not found)");
			case WidgetInputDevice.MultiKeyboard4 | WidgetInputDevice.MultiMouse4:
				return "Mouse & Keyboard 4" + ((MultiMouse.IsConnected(3) && MultiKeyboard.IsConnected(3)) ? "" : " (not found)");
			default:
				return "Touchscreen";
			}
		}
