			...
			if (inventory != null)
			{
				...
				if (activeSlotIndex >= 0)
				{
					...
					if (num == 191 && slotCount > 0)
					{
						...
						Vector3 vector = (((componentMiner.ComponentCreature.ComponentBody.CrouchFactor > 0f) ? 0.02f : 0.04f) + 0.25f * MathUtils.Saturate((num2 - 2.1f) / 5f)) * new Vector3
						{
						...
						case AimState.InProgress:
						{
							...
							if (componentFirstPersonModel != null)
							{
								...
								componentFirstPersonModel.ItemOffsetOrder = new Vector3(-0.2f, 0.3f, 0f);
						...
						case AimState.Completed:
						{
							...
							if (arrowType.HasValue)
							{
								...
								{
									vector4 = new Vector3(0.025f, 0.025f, 0.025f);
								}
								if (arrowType == ArrowBlock.ArrowType.StoneArrow)
								{
									vector4 = new Vector3(0.01f, 0.01f, 0.01f);
								}
								int value2 = Terrain.MakeBlockValue(192, 0, ArrowBlock.SetArrowType(0, arrowType.Value));
								Vector3 vector5 = Vector3.Normalize(Vector3.Cross(vector3, Vector3.UnitY));
								Vector3 vector6 = Vector3.Normalize(Vector3.Cross(vector3, vector5));
								Vector3 vector7 = m_random.Float(0f - vector4.X, vector4.X) * vector5 + m_random.Float(0f - vector4.Y, vector4.Y) * vector6 + m_random.Float(0f - vector4.Z, vector4.Z) * vector3;
								if (m_subsystemProjectiles.FireProjectile(value2, vector2, (vector3 + vector7) * num4, Vector3.Zero, componentMiner.ComponentCreature) != null)
								{
									data = BowBlock.SetArrowType(data, null);
									m_subsystemAudio.PlaySound("Audio/Bow", 1f, m_random.Float(-0.1f, 0.1f), vector2, 3f, autoDelay: true);
								}
							}
							else
							{
