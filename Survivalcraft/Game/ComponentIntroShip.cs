...
namespace Game
{
	public class ComponentIntroShip : Component, IUpdateable
	{
		...
		{
			float num = 3.5f * MathUtils.Saturate(0.07f * ((float)m_subsystemGameInfo.TotalElapsedGameTime - 6f));
			...
			matrix.Translation += num * matrix.Forward * new Vector3(1f, 0f, 1f) * dt;
			...
			if (m_subsystemTime.GameTime - m_creationTime > 10.0 && m_subsystemViews.CalculateDistanceFromNearestView(matrix.Translation) > m_subsystemSky.VisibilityRange + 10f)
			{
