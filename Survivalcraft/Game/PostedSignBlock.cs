	...
	public abstract class PostedSignBlock : SignBlock, IElectricElementBlock, IPaintableBlock
	{
		...
		public override void Initialize()
		{
			...
			for (int i = 0; i < 16; i++)
			{
				...
				Matrix matrix = Matrix.CreateRotationY((float)GetDirection(i) * (float)Math.PI / 4f) * Matrix.CreateTranslation(0.5f, 0f, 0.5f);
				if (hanging)
				{
					matrix *= Matrix.CreateScale(1f, -1f, 1f) * Matrix.CreateTranslation(0f, 1f, 0f);
				}
				m_directions[i] = matrix.Forward;
				BlockMesh blockMesh = new BlockMesh();
				blockMesh.AppendModelMeshPart(model.FindMesh("Sign").MeshParts[0], boneAbsoluteTransform * matrix, makeEmissive: false, hanging, doubleSided: false, flipNormals: false, Color.White);
				BlockMesh blockMesh2 = new BlockMesh();
				blockMesh2.AppendModelMeshPart(model.FindMesh("Post").MeshParts[0], boneAbsoluteTransform2 * matrix, makeEmissive: false, hanging, doubleSided: false, flipNormals: false, Color.White);
				...
				m_surfaceMeshes[i].AppendModelMeshPart(model.FindMesh("Surface").MeshParts[0], boneAbsoluteTransform3 * matrix, makeEmissive: false, hanging, doubleSided: false, flipNormals: false, Color.White);
