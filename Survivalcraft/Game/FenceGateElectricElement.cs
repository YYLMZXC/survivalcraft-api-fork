			...
			if (!m_needsReset)
			{
				if (num >= 10)
				{
					if (IsSignalHigh(m_voltage))
					{
						...
						SubsystemElectricity.Project.FindSubsystem<SubsystemFenceGateBlockBehavior>(throwOnError: true).OpenCloseGate(cellFace.X, cellFace.Y, cellFace.Z, !FenceGateBlock.GetOpen(data));
					}
				}
				else
				{
