...
namespace Game
{
	public static class WorldsManager
	{
		...
		public static void RepairWorldIfNeeded(string directoryName)
		{
			try
			{
				...
				if (Storage.FileExists(path))
				{
					...
					{
						TerrainSerializer14.ReadTOCEntry(stream, out var cx, out var cz, out var _);
		...
		private static string GetUnusedWorldDirectoryName()
		{
			...
			{
				string text2 = Storage.CombinePaths(Storage.GetDirectoryName(text), Storage.GetFileNameWithoutExtension(text));
				string extension = Storage.GetExtension(text);
				string text3 = string.Format("{0}{1}{2}", new object[3]
				{
					text2,
					(i > 0) ? i.ToString() : string.Empty,
					extension
				});
				if (!Storage.DirectoryExists(text3) && !Storage.FileExists(text3))
				{
					return text3;
				}
			}
		...
		private static void PackWorld(string directoryName, Stream targetStream, Func<string, bool> filter, bool embedExternalContent)
		{
			...
			using (ZipArchive zipArchive = ZipArchive.Create(targetStream, keepStreamOpen: true))
			{
				...
				if (!BlocksTexturesManager.IsBuiltIn(worldInfo.WorldSettings.BlocksTextureName))
				{
					...
					{
						Log.Warning(string.Format("Failed to embed blocks texture \"{0}\". Reason: {1}", new object[2]
						{
							worldInfo.WorldSettings.BlocksTextureName,
							ex.Message
						}));
					}
				}
				foreach (PlayerInfo playerInfo in worldInfo.PlayerInfos)
				{
					...
					{
						Log.Warning(string.Format("Failed to embed character skin \"{0}\". Reason: {1}", new object[2] { playerInfo.CharacterSkinName, ex2.Message }));
					}
				}
			}
		}
			...
			using (ZipArchive zipArchive = ZipArchive.Open(sourceStream, keepStreamOpen: true))
			{
				foreach (ZipArchiveEntry item in zipArchive.ReadCentralDir())
				{
					...
					if (text.StartsWith("EmbeddedContent"))
					{
						...
						{
							Log.Warning(string.Format("Failed to import embedded content \"{0}\". Reason: {1}", new object[2] { text, ex.Message }));
						}
					}
					else
					{
