...
public static class BevelledShapeRenderer
{
	public struct Point : IEquatable<Point>
	{
		...
		{
			if (Position == other.Position && RoundingRadius == other.RoundingRadius)
			{
	...
	public static void QueueShape(FlatBatch2D batch, IEnumerable<Point> points, float pixelsPerUnit, float antialiasSize, float bevelSize, bool flatShading, Color centerColor, Color bevelColor, float directional, float ambient)
	{
		...
		RoundCorners(TmpPoints, loop: true, pixelsPerUnit, MathF.Abs(bevelSize), TmpPositions);
		RemoveDuplicates(TmpPositions);
		TmpNormals.Count = 0;
		PathRenderer.GeneratePathNormals(TmpPositions, null, loop: true, TmpNormals);
		if (bevelColor != Color.Transparent && bevelSize != 0f)
		{
			...
			GeneratePathPoints(TmpPositions, 0f, 0f, 0f, num, bevelColor, bevelColor, bevelColor, bevelColor, 0f, float.PositiveInfinity, TmpPathPoints);
			LightPoints(TmpPathPoints, loop: true, flatShading, directionToLight, directional, ambient);
			PathRenderer.QueuePath(batch, TmpPathPoints, TmpNormals, null, loop: true, flatShading);
			...
			if (antialiasSize > 0f)
			{
				...
				GeneratePathPoints(TmpPositions, 0f, num, 0f, outerRadiusR, bevelColor, bevelColor, Color.Transparent, Color.Transparent, 0f, float.PositiveInfinity, TmpPathPoints);
				LightPoints(TmpPathPoints, loop: true, flatShading, directionToLight, directional, ambient);
				PathRenderer.QueuePath(batch, TmpPathPoints, TmpNormals, null, loop: true, flatShading);
				TmpPathPoints.Count = 0;
				GeneratePathPoints(TmpPositions, 0f, 0f, antialiasSize, 0f, bevelColor, bevelColor, Color.Transparent, Color.Transparent, 0f, float.PositiveInfinity, TmpPathPoints);
				LightPoints(TmpPathPoints, loop: true, flatShading, directionToLight, directional, ambient);
				PathRenderer.QueuePath(batch, TmpPathPoints, TmpNormals, null, loop: true, flatShading);
			}
		}
		else if (centerColor != Color.Transparent)
		{
			...
			{
				TmpPathPoints.Count = 0;
				GeneratePathPoints(TmpPositions, 0f, 0f, antialiasSize, 0f, centerColor, centerColor, Color.Transparent, Color.Transparent, 0f, float.PositiveInfinity, TmpPathPoints);
				PathRenderer.QueuePath(batch, TmpPathPoints, TmpNormals, null, loop: true, flatShading);
			}
		}
	}
	...
	public static void QueueShapeShadow(FlatBatch2D batch, IEnumerable<Point> points, float pixelsPerUnit, float shadowSize, Color shadowColor)
	{
		...
		TmpPoints.AddRange(points);
		TmpPositions.Count = 0;
		RoundCorners(TmpPoints, loop: true, pixelsPerUnit, shadowSize, TmpPositions);
		RemoveDuplicates(TmpPositions);
		TmpNormals.Count = 0;
		PathRenderer.GeneratePathNormals(TmpPositions, null, loop: true, TmpNormals);
		TmpPathPoints.Count = 0;
		GeneratePathPoints(TmpPositions, 0f, 0f, shadowSize, 0f, shadowColor, Color.Transparent, Color.Transparent, Color.Transparent, 0f, float.PositiveInfinity, TmpPathPoints);
		PathRenderer.QueuePath(batch, TmpPathPoints, TmpNormals, null, loop: true, flatShading: true);
		TmpIndices.Count = 0;
		Triangulate(TmpPositions, TmpIndices);
		int count = batch.TriangleVertices.Count;
	...
	public static void QueueShape(TexturedBatch2D batch, IEnumerable<Point> points, Vector2 textureScale, Vector2 textureOffset, float pixelsPerUnit, float antialiasSize, float bevelSize, bool flatShading, Color centerColor, Color bevelColor, float directional, float ambient)
	{
		TmpBatch.Clear();
		QueueShape(TmpBatch, points, pixelsPerUnit, antialiasSize, bevelSize, flatShading, centerColor, bevelColor, directional, ambient);
	...
	public static void QueueQuad(FlatBatch2D batch, Vector2 p1, Vector2 p2, float pixelsPerUnit, float antialiasSize, float bevelSize, float roundingRadius, int roundingCount, bool flatShading, Color centerColor, Color bevelColor, float directional, float ambient)
	{
		TmpQuadPoints.Count = 0;
		TmpQuadPoints.Add(new Point
		{
		...
		QueueShape(batch, TmpQuadPoints, pixelsPerUnit, antialiasSize, bevelSize, flatShading, centerColor, bevelColor, directional, ambient);
	}

	public static void QueueQuad(TexturedBatch2D batch, Vector2 p1, Vector2 p2, Vector2 textureScale, Vector2 textureOffset, float pixelsPerUnit, float antialiasSize, float bevelSize, float roundingRadius, int roundingCount, bool flatShading, Color centerColor, Color bevelColor, float directional, float ambient)
	{
		TmpQuadPoints.Count = 0;
		TmpQuadPoints.Add(new Point
		{
		...
		QueueShape(batch, TmpQuadPoints, textureScale, textureOffset, pixelsPerUnit, antialiasSize, bevelSize, flatShading, centerColor, bevelColor, directional, ambient);
	}

	private static void RemoveDuplicates(DynamicArray<Vector2> positions)
	{
		int count = 0;
		Vector2? vector = null;
	...
	private static void LightPoints(DynamicArray<PathRenderer.Point> points, bool loop, bool flatShading, Vector2 directionToLight, float directional, float ambient)
	{
		if (flatShading)
		{
	...
	private static void GeneratePathPoints(DynamicArray<Vector2> positions, float innerRadiusL, float innerRadiusR, float outerRadiusL, float outerRadiusR, Color innerColorL, Color innerColorR, Color outerColorL, Color outerColorR, float lengthScale, float miterLimit, DynamicArray<PathRenderer.Point> result)
	{
	...
	private static void RoundCorners(DynamicArray<Point> points, bool loop, float pixelsPerUnit, float bevelSize, DynamicArray<Vector2> result)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			RoundCorner(point.Position, point2.Position, point3.Position, point2.RoundingRadius, point2.RoundingCount, pixelsPerUnit, bevelSize, result);
		}
	}

	private static void RoundCorner(Vector2 p0, Vector2 p1, Vector2 p2, float radius, int count, float pixelsPerUnit, float bevelSize, DynamicArray<Vector2> result)
	{
