...
public class SurvivalCraftModInterface(IModLoader parent) : ModInterface(parent)
{
	...
	public override string[] AvailableHooks =>
		...
		/// <param name="z"/>
		/// <param name="value"/>
		/// <param name="placed">为true则不执行原放置操作</param>
		/// <returns></returns>
		...
		/// 动物吃掉落物时执行
		/// </summary>
		public virtual void OnEatPickable(ComponentEatPickableBehavior eatPickableBehavior, Pickable pickable, out bool dealed)
		{
			dealed = false;
		}
